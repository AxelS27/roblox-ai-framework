...
* **[Starter Tutorial & Onboarding Pattern](file:///d:/Experiments/Roblox%20AI%20Framework/.agents/patterns/tutorial-pattern.md)**: Declarative step-by-step tutorial engine, 3D objective highlights, and DataStore onboarding persistence.
...
11. **UI Placement Coordination & Mandatory UIAutoBinder Mandate**: All custom ScreenGuis MUST be decorated and animated using `ThemeManager.luau` and `UIAutoBinder.luau`. (1) **No Plain Navy Rectangles**: Flat navy/blue boxes with static text are strictly FORBIDDEN. All UI frames MUST use frosted glassmorphism (`ThemeManager.applyGlassmorphism`), rounded corners (`UICorner`), subtle white stroke borders (`UIStroke`), and HSL gradient fills (`ThemeManager.applyBarGradient`) for progress bars. (2) **Mandatory Animations & Sound**: Every interactive button MUST be bound via `UIAutoBinder.decorateButton` or `UIAnimate.bindHoverScale`, `UIAnimate.bindStrokeScale`, and click audio feedback. Static un-animated buttons are strictly FORBIDDEN. (3) **Roblox Native Safe Zones**: Respect the top-right corner for native `leaderstats` and top-left for native Chat UI. (4) **Anti-Overlap Margin**: Maintain a minimum vertical separation gap of `40` pixels between independent HUD widgets to guarantee zero visual collisions.
...
13. **Mandatory Interactive Starter Tutorial & Onboarding Engine**: EVERY game generated MUST include a 3 to 5 step interactive starter tutorial sequence using `TutorialEngine.luau`. First-time players must be guided step-by-step through core mechanics (e.g., 1. Walk/Camera controls, 2. Core mechanic / place block / eat flora / attack dummy, 3. Open UI menu, 4. Collect tutorial completion reward). Tutorial completion state MUST be saved in player data (`ProfileService.TutorialCompleted`) so returning players skip it automatically.
...
15. **High-Fidelity Level Design & Non-Flat Topography Invariant**: Staging flat ground baseplates with bright yellow or neon cyan primitive blocks is STRICTLY FORBIDDEN. All map environments staged in Roblox Studio MUST have natural terrain elevation (hills, valleys, riverbeds). When generating vegetation, rocks, and props, search and audit models from the Roblox Creator Marketplace/Toolbox per **Constraint 14**. Randomize orientation (`CFrame.Angles(0, math.rad(math.random(0, 360)), 0)`) and scale factors (`0.8` to `1.3`). Setup atmospheric post-processing in `Lighting` (`Future` technology, `Sky`, `Atmosphere`, `ColorCorrection`, `SunRays`, `Bloom`) and spatial 3D audio.
16. **Custom Mobile HUD Layout & Device Clearance UX**: When developing cross-platform support, DO NOT use unstyled default `ContextActionService` touch buttons for mobile players. (1) **Device Filtering**: Touch HUDs MUST ONLY set `Enabled = true` when `UserInputService.TouchEnabled == true`. On PC (Mouse & Keyboard), Mobile Touch HUDs MUST be hidden (`Enabled = false`). (2) **Roblox Native Touch Jump Clearance**: Mobile action buttons MUST NOT overlap Roblox native Jump button (bottom-right `UDim2.new(1, -25, 1, -25)`). Custom mobile touch arc containers MUST be positioned at least `130`px left and `120`px up (`UDim2.new(1, -130, 1, -120)`) to guarantee zero thumb collision.
...
19. **Mandatory 3-Point Task Verification Guardrail**: NO task can be marked `🟢 Completed` merely by compiling without errors. Every task MUST pass an explicit 3-Point Quality Audit before completion:
    - **Point 1: Visual & Aesthetic Polish**: `ThemeManager` glassmorphism, HSL gradients, and `UIAutoBinder`/`UIAnimate` hover/click animations are verified active on all UI components. Maps use elevated topography, audited props, and Future lighting.
    - **Point 2: End-to-End System Connectivity**: Every button click or physical interaction fires `Net` remotes and triggers functional server state changes (saving data, updating leaderstats, spawning items).
    - **Point 3: Console & Hierarchy Audit**: Exactly 0 red error logs or warn traces, no overlapping UI frames, and strict Z-Index layering.
20. **Anti-Shortcut & Granular Step-by-Step Execution Invariant**: The AI assistant MUST NEVER shortcut, compress, summarize, skip steps, or write placeholders (e.g. `-- TODO: implement remaining logic`, `-- ... rest of code`, or high-level task summaries). Every task file, GDD document, and Luau script MUST be detailed and executed with 100% full technical completeness step-by-step. Skipping requirements, writing hollow code stubs, or glossing over specified parameters is strictly FORBIDDEN.
21. **Contextual UI State Lifecycle Invariant**: ScreenGuis MUST NOT be permanently enabled across all game states. Regardless of game genre (RPG, Survival, Simulator, Racing, Tycoon, Battle Royale, etc.), every custom ScreenGui MUST register with `UIManager.register(gui, allowedStates)` so UI visibility transitions dynamically based on active gameplay phase/context. Game-phase tools, hotbars, combat action HUDs, and inventory grids MUST ONLY be visible during active gameplay states (hidden in Lobby/Intermission). Lobby HUDs and main menu panels MUST ONLY be visible in Lobby/Menu states.
